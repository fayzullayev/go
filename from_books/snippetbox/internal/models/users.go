...
func (user *UserModel) Insert(name, email, password string) error {
...
func (user UserModel) Authenticate(email, password string) (int, error) {
	...
	err := user.DB.QueryRow(stmt, email).Scan(&id, &hashedPassword)
...
func (user UserModel) Exist(id int) (bool, error) {
	...
	err := user.DB.QueryRow(stmt, id).Scan(&exist)
